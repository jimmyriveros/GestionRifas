...
exactamente el mismo concepto, y tener dos enumerados para lo mismo sería la primera forma de que se
separen.
...
| Invariante | Cómo |
|---|---|
| El estado del premio **es** el de su versión vigente | FK compuesta `(current_version_id, id, status)` → `(id, prize_id, status)` |
| Un premio **nace** con su versión 1 y su puntero **solo avanza** | Disparador `raffle_prizes_guard` |
| Una versión **no se modifica ni se borra**, y la cadena es lineal | Disparador `raffle_prize_versions_guard` + `UNIQUE (prize_id, version_number)` |
| Toda versión tiene **al menos un período** | Disparador de restricción **diferido** `raffle_prize_versions_require_rules` |
