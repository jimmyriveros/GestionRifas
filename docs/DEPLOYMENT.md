...
| Elemento | Estado |
|---|---|
| Migraciones (**48** aplicadas, hasta `0048`; **`0049` pendiente**) | Aplicadas y verificadas con `npm run verify:remote`. La cifra se quedó en «21» durante varias promociones; se corrigió al aplicar `0040` (2026-08-31) y `0041` (2026-09-01, D-156), y se mantiene desde entonces: `0042` (09-01), `0043`+`0044` (09-02), `0045` (09-02), `0046` (09-03), `0047` (09-03, D-168) y **`0048` (09-05, D-169)**, esta última con la migración aplicada **antes** del despliegue. ⚠️ **`0049` (D-170, paz y salvo) NO está aplicada.** Es la primera migración desde `0027` que **escribe datos** —la carga inicial que da por entregadas las boletas ya vendidas—, así que su promoción exige, además del respaldo de siempre: sonda de solo lectura **antes** (total de boletas, asignadas, cuántas recibirán la marca, distribución por estado, y totales de ventas, abonos y saldos), `db push --dry-run` comprobando que **solo** aparece `0049`, la migración **antes** del despliegue, y la **misma sonda después** para confirmar que ninguna cifra financiera se movió |
