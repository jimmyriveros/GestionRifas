...
| I-032 | Una corrida completa de E2E falló en dos pruebas que no volvieron a fallar | Info | En la primera corrida tras reiniciar Docker fallaron `payments.spec.ts:210` y `tour-responsive.spec.ts:29`, ambas por tiempos. No se reprodujeron: la suite completa pasó 152/152 después, y `payments.spec.ts` pasó 17/17 dos veces más, con y sin los cambios del recorrido. Se deja anotado porque un fallo intermitente documentado es información y uno omitido es una trampa |
