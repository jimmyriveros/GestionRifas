| R-13 | Sesión de usuario recién desactivado que sigue operando | Media | Alto | `is_active` verificado en cada request y dentro de `current_org_ids()` (D-006) | ✅ Verificado en Fases 1 y 2 || R-08 | Trigger de recálculo de `paid_amount` que no cubre algún camino (borrado, anulación, actualización) | Media | Alto | Cubre INSERT/UPDATE/DELETE de asignaciones y el cambio de `voided_at`; prueba que compara `paid_amount` con la suma real | ✅ Verificado en Fase 2 || R-04 | Función `SECURITY DEFINER` sin `search_path` fijo: secuestro de esquema | Media | **Crítico** | `SET search_path = public, pg_temp` en las 20 funciones + prueba de catálogo | ✅ Verificado en Fase 2 · revisar en 7 y 9 || R-03 | Vista creada sin `security_invoker`: fuga de datos entre vendedores | Media | **Crítico** | Todas las vistas lo declaran + prueba de catálogo automatizada que falla si aparece una sin él | ✅ Verificado en Fase 2 · revisar en 7 y 9 || R-02 | Sobrepago por concurrencia: dos abonos simultáneos sobre la misma boleta | Media | Alto | `SELECT … FOR UPDATE` ordenado por `ticket_id` + snapshot nuevo por sentencia + `CHECK (paid_amount <= sale_price)` | ✅ Probado con dos pagos concurrentes reales (Fase 2) || R-01 | Recursión infinita en políticas RLS al consultar `memberships` desde políticas de `memberships` | Alta | Alto | Funciones auxiliares `SECURITY DEFINER STABLE` que omiten RLS (SECURITY §4.1) | ✅ Verificado en Fase 2 |# PROBLEMAS CONOCIDOS Y RIESGOS

- **Versión:** 1.2 · **Actualizado:** 2026-08-03 (Fase 2)
...
| I-002 | Docker Desktop no estaba disponible | Media | ✅ Resuelto (Fase 2) | Instalado por el usuario (Docker 29.6.2). La Fase 2 lo usó para Supabase local, `db reset`, las 111 pruebas de base de datos y la generación de tipos |
...
| I-006 | `supabase gen types typescript --db-url` exige Docker aunque apunte a una base remota | Media | ✅ Resuelto (Fase 2) | Con Docker instalado, los tipos se generan con `--local` desde el esquema real. `database.types.ts` ya no se mantiene a mano (cierra D-034 y DT-10) |
...
| I-010 | **Un carácter `\r` se coló dentro de `SEED_DEFAULT_PASSWORD` en `.env.local`** | Media | ✅ Resuelto (Fase 2) | La contraseña se generó con `openssl rand -base64 … > archivo` en Git Bash sobre Windows; el archivo quedó con CRLF y `tr -d '\n'` eliminó solo el salto de línea, dejando el retorno de carro **dentro** del valor entrecomillado. Efecto: los scripts (que leen con dotenv) fijaban en Supabase una contraseña terminada en `\r`, imposible de teclear en el navegador, y el login fallaba con `invalid_credentials` mientras que por API funcionaba. Diagnóstico: comparar la longitud del valor con la de la contraseña visible. **Lección para fases futuras: nunca construir un valor de `.env` con redirecciones de shell en Windows sin normalizar los saltos de línea.** |

I-002 e I-006 quedaron **resueltos** al instalar Docker: la Fase 2 usó Supabase local para migraciones,
pruebas y generación de tipos. No queda ningún problema que bloquee la Fase 3.
...
| ~~DT-10~~ | ~~`database.types.ts` escrito a mano~~ | **Saldada en la Fase 2**: se genera con `supabase gen types typescript --local` desde el esquema real | — |
...
| 2026-08-03 | 2 | I-002 e I-006 resueltos (Docker). Agregado I-010 (retorno de carro dentro de un valor de `.env.local`). DT-10 saldada. R-01, R-02, R-03, R-04, R-08 y R-13 verificados con pruebas automatizadas contra base de datos real. |
