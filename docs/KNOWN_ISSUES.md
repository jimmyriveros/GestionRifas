...
**Actualizado:** 2026-08-03 (Fase 2). Este documento **no oculta errores**.
...
| I-007 | `auth.admin.createUser({password})` no deja la contraseña usable de inmediato | Mitigado | Verificado: el primer `signInWithPassword` falla; un `updateUserById` posterior con la misma contraseña sí funciona. `scripts/seed.ts` ya lo aplica. **Replicar este flujo de dos pasos al invitar usuarios en la Fase 3** |
...
**Sin bloqueantes para la Fase 3.**
...
| R-05 | Navegador congelado con 1.000 filas | Virtualización, validación diferida, lotes de 100 | ⬜ Control en F3 |
...
| R-10 | Consultas N+1 en listados | Vistas agregadas + `select` con relaciones; `EXPLAIN ANALYZE` | ⬜ Control en F3/F6 |
| R-11 | Errores de PostgreSQL expuestos al usuario | `mapPgError` traduce a mensajes en español | ⬜ Ampliar en F3 |
...
| R-14 | Fallo parcial en carga masiva | RPC por lote que devuelve insertados y conflictos sin abortar | ✅ Implementado F2 |
...
| DT-11 | Playwright no instalado | No hay specs E2E que lo justifiquen aún | F3 en adelante |
