...
| `unit/sidebar-preference.test.ts` | Cómo se combinan la preferencia guardada y el sitio disponible (D-131): una cookie ausente o manipulada abre la barra; sin sitio se cierra **sin borrar** la preferencia; la cookie lleva `path`, caducidad y `secure` solo en HTTPS. Y el guardián que **no puede escribirse de otra forma**: que el punto de corte de `globals.css` y el de TypeScript sigan siendo el mismo número, porque el CSS no se importa |
| `e2e/menu-lateral.spec.ts` | La barra lateral de escritorio (D-131). Cada prueba **fija su ventana**, porque el ancho es lo que se prueba: a 1.600 abierta con sus ocho nombres; cerrarla a mano deja los iconos y le da a la tabla **más de 150 px**; sigue cerrada al navegar **y tras recargar** (la cookie); el globo aparece con el ratón **y con el foco**; se estrecha a 1.360 (208 px) y a 1.100 se cierra sola sin perder la preferencia, que vuelve al ensanchar; una barra cerrada a mano sigue cerrada aunque sobre sitio; y a 1.100 el botón está `aria-disabled`, lo explica en su globo y **no abre nada ni forzando el clic**. Además, a 1.360 —el ancho más apretado— se mide que **ningún nombre se parte en dos líneas ni se recorta** |
