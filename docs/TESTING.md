...
`tests/db/prize-award-history.test.ts` — **69** pruebas, migraciones `0067` a `0071`, **solo en local**.
...
| **H7** (4) | Las **15** funciones de `0067`, `0068` y `0070` están clasificadas y su EXECUTE es **exactamente** el de la lista; toda función que crean las migraciones `0067` a `0071` está en la lista **y ninguna más**; la tabla tiene RLS forzada y concede **solo `SELECT`**; y nada de la suite tocó abonos, asignaciones ni movimientos de comisión |
