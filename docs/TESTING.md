...
| `tests/unit/raffle-activation.test.tsx` | **10** (nuevo, corrección de D-202) | `draftActivation` y el detalle montado: un borrador **configurable no presenta «Activar rifa»** y **presenta «Revisar y activar»** hacia `/review`; un borrador **heredado conserva «Activar rifa»** con su confirmación y llama a `changeRaffleStatus`; sin la capacidad no hay ninguna de las dos; y las demás transiciones —cerrar, anular, **reabrir solo el Dueño**— no cambian |
