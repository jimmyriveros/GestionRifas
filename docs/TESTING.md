...
### 4.11 Premios configurables por rifa (BR-J01..BR-J16, BR-R12; D-199 a D-206)
...
| `tests/db/raffle-date-notices.test.ts` | **11** (nuevo, D-206, BR-R12) | **El aviso de las fechas de una rifa activa, en la base.** El sistema extiende el fin: **un aviso por membresía activa**, ninguno a la inactiva ni a otra organización, un solo evento y la bitácora con cuántos salieron; desde la sesión del **Dueño**, todos menos él, y cada quien lee **solo el suyo** con su sesión; un vendedor no puede cambiarlas. **Cuándo no:** guardar las mismas fechas —el reintento de la pantalla—, cambiar otra cosa, un borrador, una rifa cerrada o una que se cierra en el mismo cambio. **Atómico:** un cambio que rechaza el disparador de premios no deja aviso, y uno deshecho tampoco; **dos cambios iguales a la vez** dan un solo evento; dos distintos, dos; y el índice único rechaza un duplicado del mismo evento. **Lo que dice:** exactamente la rifa y las fechas, nada de la cartera, y el texto de la campana |
| `tests/unit/raffle-dates-notification.test.ts` | **7** (nuevo, D-206, BR-R12) | El texto del aviso: fin, inicio, las dos —en el mismo año, cruzando de año y un solo día—, datos incompletos o raros sin inventar fechas, y nada de la cartera ni enlace. Y la pantalla de editar: la frase **solo** con la rifa activa y una fecha distinta de la guardada |
...
| `tests/e2e/rifa-fechas-aviso.spec.ts` | **3** (nuevo, D-206, BR-R12) | Rifas de **2063**, borradas al final, y cada fecha se escribe **cuando React ya hidrató el campo** —antes, la hidratación repone la guardada (§5.3)—: la pantalla de editar una rifa activa **no dice nada** hasta cambiar una fecha, lo dice al cambiarla, lo retira al volver a la guardada, y guarda; el vendedor lee **un** aviso «Cambiaron las fechas de …: ahora termina el 21 de diciembre de 2063.» y el Dueño que guardó no lo tiene; un borrador no anuncia nada |
