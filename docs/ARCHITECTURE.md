...
| Registrar abono (Seller) | El origen validado: boleta, cliente, «Mis pagos» o el panel; si no hay, `/seller/payments` (D-135) |
...
> 320 px. ✅ **En producción desde el 2026-09-12** (Etapa 7, D-193), con la `0051` aplicada al
> proyecto real.
>
...
> ✅ **En producción desde ese mismo día** (Etapa 7, D-193): el proyecto real tiene las tres, más la
> `0055` que instala `pg_net` (I-112).
>
...
| `lib/og-renderer.ts` · `instrumentation.ts` | **D-223, D-224, D-226.** `instrumentation` registra, una vez y solo en Node, un gancho de módulos que le da a `@vercel/og` —y solo a él— `sharpForOg` en lugar de `sharp`. Hace la misma llamada que `@vercel/og` (`sharp(svg).resize(w).png().toBuffer()`, sin opciones), así que recibe el SVG ya saneado por su Satori (GHSA-vcvr-r3jv-pc5j) y no rodea el parche. **`sharp` se carga al generar la primera imagen, nunca al importar el archivo** (I-167, D-224): un `import` estático de `sharp` aquí haría que una instancia sin `sharp` respondiera 500 en todas las rutas; lo vigila `og-renderer-sin-sharp.test.ts`. Sin `sharp`, la imagen falla: no se cae a resvg (D-224). **Ya no hay proceso hijo** (D-226): existía por I-163, y desde Next 16.3.6 el optimizador de `/_next/image` vuelve a habilitar el cargador SVG; la vigía de `weekly-results-image-sharp.test.ts` falla si una versión futura lo quita otra vez, y entonces se reevalúa I-163 en vez de relajar la prueba. **No se desbloquea ningún cargador** desde nuestro código |
