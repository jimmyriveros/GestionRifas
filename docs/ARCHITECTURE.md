...
| `ConfirmDialog` | Confirmación de acciones sensibles (anular, desactivar, aprobar). Sus dos botones traen el **suelo táctil de 44 px** en el teléfono —36 desde `sm`— porque lo pone `AlertDialogAction`/`AlertDialogCancel`, no esta pantalla ni sus consumidores (D-177, I-102). **No le pases `size` a los botones de un diálogo de confirmación** |
