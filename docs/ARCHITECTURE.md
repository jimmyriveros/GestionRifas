...
| `lib/og-renderer.ts` · `instrumentation.ts` | **I-163, D-223.** `instrumentation` registra, una vez y solo en Node, un gancho de módulos que le da a `@vercel/og` —y solo a él— `sharpForOg` en lugar de `sharp`. Usa el `sharp` del proceso y, si el optimizador de `/_next/image` ya le bloqueó el cargador SVG, rasteriza en un proceso hijo. **No se desbloquea ningún cargador**; no lo quites sin repetir las dos pruebas de D-223. El hijo lo gobierna `runChild`: cualquier fallo rechaza la imagen una sola vez, cierra el hijo y sus canales y no deja ninguna excepción sin capturar (corrección de D-223, `og-renderer-child.test.ts`). **`sharp` se carga al generar la primera imagen, nunca al importar el archivo** (I-167, D-224): un `import` estático de `sharp` aquí haría que una instancia sin `sharp` respondiera 500 en todas las rutas; lo vigila `og-renderer-sin-sharp.test.ts` |
