...
| Ahí, el orden que pone la BÚSQUEDA en «Mis boletas», donde nadie pidió columna | **«Las que mejor coinciden»** (D-216) | «Relevancia», que es palabra de buscador; «Más recientes primero», que ahí es falso: buscando, la lista no sale por fecha |
| Ahí, un orden que la consulta SÍ aplica y el teléfono no ofrece —llegó de la pantalla grande o de un enlace guardado— | Se **describe**, no se esconde: **«Rifa, de la Z a la A»**, **«Vendedor, de la A a la Z»**, y un estado por su primera etiqueta, **«Estado de la boleta, primero Borrador»** (D-216). **Buscando, esa lista la ordena otra consulta y la primera es otra**: «primero **Asignada**», «Estado de pago, primero **Pagada**» (D-218) | Caer al orden por defecto, que anunciaría uno y enseñaría otro; y «de la A a la Z» para un estado, que se ordena por su lista de valores y no por el alfabeto |
| Lo mismo en «Boletas» del **personal**, donde la consulta compara los estados de otra forma | La etiqueta del estado que **de verdad sale primero ahí**: **«Estado de la boleta, primero Asignada»**, **«Estado de pago, primero Pagada»**, **«Paz y salvo, primero Por entregar»** (D-217). En el teléfono del personal se ofrecen solo **Boleta**, **Rifa** y **Vendedor** | Copiar la frase del vendedor, «primero Borrador», que ahí sería falsa; y cualquier opción de **cliente** o de **dinero**, que el personal no ve (D-198) |
| La frase del orden, cuando no cabe en una línea | **Baja a la segunda**, entera (D-165, D-217) | Recortarla con puntos suspensivos: «…primero Pendi…» no dice ningún orden |
...
| Las frases de cada orden que se ofrece en el teléfono, la del orden por defecto y la de uno que la consulta aplica sin que el teléfono lo ofrezca | `src/features/tickets/sort-options.ts` y `src/features/clients/sort-options.ts` (D-215, D-216), **cada una con las suyas**. Ahí viven también `ticketDefaultSortLabel` —que dice «Las que mejor coinciden» mientras hay búsqueda— y `describeTicketSort` / `describeClientSort`. Las del **personal** están en el mismo archivo de boletas: `STAFF_TICKET_SORT_OPTIONS` y `describeStaffTicketSort` (D-217). Una prueba comprueba que ninguna opción pida algo que la consulta rechace, y otra que las del personal no nombren cliente ni dinero |
| Nombre de lo que cuenta cada listado en su paginación | `src/lib/constants.ts` (`LIST_ITEM_LABELS`, D-111) |
| El historial de abonos de la ficha del cliente: su estado vacío, «Esa página no existe», «El historial tiene 130 abonos en 6 páginas.» e «Ir a la primera página» | `src/features/payments/components/ClientPaymentsHistory.tsx` (D-219). Su barra cuenta **abonos** (`LIST_ITEM_LABELS.clientPayments`), no pagos: lo que dice el título de su sección |
...
| Las negativas y los avisos del cargador de premios reconocidos —destino y proyecto esperado, organización dos veces, huella, entradas y el contraste de cada informe— | `scripts/record-prize-awards-guard.ts`, **todas juntas** (D-208, Etapa 4); el script solo pinta el informe, los recuentos y cómo terminó. Las de destino son las de la transición, **reutilizadas**. Las lee quien opera, y cada una dice si **se escribió algo o no** |
| Los de las herramientas de puerta —la orden, el proyecto esperado y los motivos para detener— | `scripts/gate-db.ts` y `scripts/gate-compare.ts` (D-208, Etapa 4). Nombran tablas y filas por su clave, nunca un dato de cliente |
