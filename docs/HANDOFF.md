...
| Remoto | `github.com/jimmyriveros/GestionRifas`. `main` local y `origin/main` coincidían en **`16a1b74`** el 2026-08-21, después del `push --force-with-lease`; a partir de ahí vuelve a comprobarse con Git, no se asume por este texto |
...
| **Rendimiento — navegación** | Medida la cadena completa desde el clic (D-104, 2026-08-22): local **826 → 124 ms**, Vercel con la función caliente **~840 → ~350 ms**. Se quitaron los `loading.tsx` (imponían ~300 ms de espera) y la precarga de las filas de tabla. **Lo que queda de los ~3 s es el arranque en frío de Vercel (I-067) y no se arregla con código**: hay que activar Fluid Compute |
...
| Pruebas | **320 unitarias**, **518 de base de datos** (+6 de `read-performance`) y **274 E2E**, revalidadas el 2026-08-22 con `verify` en verde. La suite de base de datos aguanta pasadas seguidas sobre la misma base (I-057 e I-059); la de E2E **no**, y hay que sembrarla limpia antes de una pasada completa. CI en GitHub Actions desde la Fase 8 |
...
| Advertencias | **1)** **No vuelvas a añadir un `loading.tsx`** sin medir: cuesta ~300 ms de espera por el fallback de Suspense. El aviso lo da `NavPending`. **2)** **No pongas `<Link>` a pelo en una fila de tabla**: usa `RowLink`. **3)** Una pantalla de «no encontrado» ahora se pinta **sin el menú lateral** y responde **404** de verdad; una prueba que lea `main` no encontrará nada, hay que leer el `body`. **4)** **Lo que quedan de los 3 s es I-067 y no se arregla con código**: hay que activar **Fluid Compute** en el proyecto de Vercel. **5)** El doble `getUser()` por petición (~80 ms) se dejó a propósito: es la validación de sesión en capas y no se toca por 80 ms |
| Publicación | Desplegado el 2026-08-22 con autorización expresa. Vercel `READY` sobre **`97e1984`** (`dpl_Fzq9LQrgnKnfWbU1XNGP9zjwTmQC`), alias `gestion-rifas.vercel.app`. Sin migraciones: este relevo no toca la base de datos |
| Pendiente | **1)** **Activar Fluid Compute** (I-067): es lo único que quita los segundos que quedan. **2)** Rotar la contraseña de las cuentas de demostración (I-066). **3)** I-062 e I-063, los techos de escala de D-102. **4)** La columna «Abono» del importador sigue sin empezarse |
| Git | Rama `main`, de `0e99da9` a **`97e1984`**, empujado a `origin/main` |
