...
| **Barra de selección en el teléfono** | Desde el 2026-08-24 (D-110) el hueco de esa barra lo reserva `--selection-bar-space` al **fondo** del contenido, no un div vacío en medio de la lista, y la barra va envuelta en `display: contents` para que el margen del `space-y-6` no la levante sobre la navegación. Se acabaron los 80 px en blanco al marcar y la paginación tapada. **Escritorio no cambia.** **Sin desplegar**: commit local (`ARCHITECTURE` §8.8) |
...
| Último cambio promovido | **`f9d5b20`** — **la misma cabecera en el portal administrativo** (D-109), 2026-08-24; sin migración, CI 2/2 y despliegue verificado por SHA |
...
| Remoto | `github.com/jimmyriveros/GestionRifas`. `main` local y `origin/main` coincidían en **`f9d5b20`** el 2026-08-24, tras el push de D-109; a partir de ahí vuelve a comprobarse con Git, no se asume por este texto |
...
| Pruebas | **325 unitarias**, **518 de base de datos** (+6 de `read-performance`) y **292 E2E** (242 escritorio + **50 móvil**, D-110), revalidadas el 2026-08-24 con `verify` en verde. La suite de base de datos aguanta pasadas seguidas sobre la misma base (I-057 e I-059); la de E2E **no**, y hay que sembrarla limpia antes de una pasada completa. CI en GitHub Actions desde la Fase 8 |
...
## 1.a Último relevo significativo — el hueco de la barra de selección múltiple (2026-08-24)
...
| Publicación | **No desplegado.** Commit local, sin push. Requiere autorización expresa del dueño |
| Pendiente | Lo de siempre (I-066, I-062, I-063, columna «Abono» del importador). De este trabajo: **nada bloqueante**. Falta que el dueño lo mire en **un teléfono real** y autorice el push |
| Git | Rama `main`, base observada `fb91ac1`, árbol limpio antes de empezar |
