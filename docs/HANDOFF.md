...
| Último cambio promovido | **`a0bbc97`** — **Fecha ya no tapa Método en «Registrar abono»** (D-139, I-079), 2026-08-29; sin migración, CI 2/2, despliegue verificado por el identificador `d441b021c0eb` y por `webkit-date-and-time-value` |
...
| Pendiente | Verificar el despliegue en vivo (cabeceras, 401 sin secreto, identificador de versión). Lo de siempre: I-077, I-072, I-074, I-075, I-068, I-062, I-063. I-081 se orquesta, no se «arregla» evadiendo. I-082 es el tope de precisión de Hobby |
| Publicación | Autorizada. Migraciones **ya** en el proyecto real. Frontend y cron: con el push a `main` |
| Git | Rama `main`, sobre `59b909c` (Etapa 5). Árbol con `crons` Hobby y docs; `ResultadosLoterias.txt` y `prueba-abono.csv` siguen sin seguimiento |
