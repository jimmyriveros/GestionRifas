...
| Git | Rama `main`, de `3f82c42` a `463b0ef`. Se empujó **solo la rama**. ⚠️ **El historial se reescribió una vez**: el asunto del commit de la funcionalidad salió como un `@` suelto —resto de la sintaxis de aquí-documento de PowerShell— y se corrigió a petición del dueño, así que los tres commits cambiaron de SHA (`df7f1a7`→`e1b2fe1`, `ebe797b`→`0ebc3e7`, `5267107`→`463b0ef`). El árbol resultante es **idéntico byte a byte** al anterior. Rama de respaldo local: `respaldo-antes-de-reescribir` |
