...
| Último cambio promovido | **`9e72fca`** — **la cuadrícula 2 × 2 de «Información general»** (D-113.c), 2026-08-25; sin migración, CI 2/2 y despliegue verificado por SHA. Se comprobó además que las huellas CSS del build anterior **desaparecieron** |
...
| Pruebas | **372 unitarias** (2026-08-26, +13 de la PWA), **518 de base de datos** (+6 de `read-performance`) y **294 E2E**. ⚠️ Las de base de datos y las E2E se revalidaron por última vez el **2026-08-25**: en la sesión del 2026-08-26 **no se pudieron ejecutar** porque Docker no estaba levantado (**I-073**). La suite de base de datos aguanta pasadas seguidas sobre la misma base (I-057 e I-059); la de E2E **no**, y hay que sembrarla limpia antes de una pasada completa. CI en GitHub Actions desde la Fase 8 |
...
## 1.a Último relevo significativo — la aplicación se puede instalar (2026-08-26)
...
| Verificación | `npm run verify` **en verde**: `typecheck` ✅, `lint` **0 errores** (los 2 avisos de siempre), **372/372** unitarias (**+13** nuevas) y `build` ✅ · Comprobado **a mano contra `next start`** (build de producción): worker activo en el alcance `/`, **cero errores de consola**, manifiesto en `application/manifest+json`, las **dos** cachés con **20 entradas** de las que **todas** son `/_next/static/…` salvo `/offline` y el manifiesto, un `?_rsc=` pedido a mano **no** quedó guardado · **Sin conexión**: se paró el servidor y `/seller/tickets?q=1234` sirvió la pantalla de reserva **conservando la dirección original** y con React hidratado · **Actualización**: se cambió el worker, apareció el aviso, la página **no** se recargó sola, y al pulsar «Actualizar» se activó, recargó y borró la caché anterior · **Sin desbordamiento horizontal** a 320, 375, 430 y 1.366 px |
| Advertencias | **1) `test:db` y `test:e2e` NO se ejecutaron: Docker no estaba levantado** (I-073). Cinco pruebas E2E cubren justo lo que D-120 tocó —`seleccion-multiple`, `seleccion-movil`, `owner-bulk`, `tour`, `tour-responsive`— y ninguna corrió. **Levanta Docker y córrelas antes de promover.** **2)** Se encontró un fallo **grave y anterior** a este trabajo: **`/forgot-password` no ejecuta nada de su JavaScript en producción** y el correo escrito acaba en la URL (**I-070**). **No se corrigió**, porque toca autenticación; el arreglo es una línea y está escrito en la ficha. **3)** Los iconos son un **marcador de posición**, no la marca del negocio (**I-071**). **4)** Al registrar el worker en local hay que usar **`npm run build && npm start`**: en `next dev` se desregistra a propósito, porque Turbopack sirve fragmentos **sin huella** y guardarlos dejaría a quien programa mirando código viejo. **5)** Nunca añadas al matcher de `src/proxy.ts` nada que no sea un archivo estático y público: lo que entra ahí se queda sin comprobación de sesión |
| Pendiente | Ejecutar `test:db` y `test:e2e` con Docker levantado. Decidir **I-070** (el fallo de «Recuperar contraseña»), **I-071** (el logo) e **I-072** (si `font-mono` debía ser Geist Mono). **No se ha desplegado**: requiere autorización expresa. Lo de siempre: I-068, I-066, I-062, I-063 |
| Git | Rama `main`, base observada `c2a519c`. **Sin desplegar y sin empujar** |
