...
| **Menú lateral de escritorio** | Desde el 2026-08-28 (D-131) la barra lateral mide **232 px** en una ventana amplia, **de 208 a 232** entre 1.360 y 1.600 px, y **56 px —solo iconos—** por debajo de 1.360, sea porque la persona la cerró con el botón nuevo o porque no cabe. La preferencia viaja en la cookie `rifas.sidebar` y la lee el servidor, así que no hay parpadeo. **El teléfono no cambia**: sigue la barra inferior de D-106. Sin migración, sin consultas nuevas, sin cambios de reglas ni de rutas. **⚠️ NO desplegado** |
...
| Pendiente | **1)** **Desplegar**, que necesita autorización. Sin migración: la reversión sería un Instant Rollback sin nada que deshacer en la base. **2)** Verificación visual con sesión real, que un agente no hace en producción (I-066): abrir y cerrar la barra y mirar «Boletas» en una ventana de portátil. **3)** El paso del recorrido guiado que explica la barra lateral apunta a un carril de iconos cuando la ventana es estrecha; el texto sigue siendo cierto y **no se tocó**. **4)** Lo de siempre: I-077, I-072, I-074, I-075, I-068, I-066, I-062, I-063 |
| Git | Rama `main`, sobre `8809c35`. **Sin desplegar** |
