...
- **Actualizado:** 2026-09-24 — **auditoría visual y listas, D-211 a D-223, SOLO EN LOCAL** (mantenimiento
  ...
  **(1) Funcionalidades:** foco al cerrar una confirmación (D-212); las siete listas **ordenan y paginan en
  ...
  semana» sale aunque el optimizador de imágenes haya trabajado antes en el mismo servidor (D-223, I-163).
  **(2) Pruebas (D-223, la última):** `verify` **exit 0** (1.645 unitarias) · `test:db` **1.444 + 1 omitida**,
  59/59 · **E2E completa en frío y desde base limpia: 908/909**, con el único fallo en I-090, anterior al lote. Detalle y errores encontrados, en
  ...
  **(5) Problemas que permanecen:** I-163 **corregido en local** y sin comprobar en Vercel (D-223); el ACL de `search_tickets` por confirmar (I-132); **I-159**, **I-160**, **I-161**, **I-151**
...
- Antes — 2026-09-19, más tarde — **D-210 EN PRODUCCIÓN** (`9acbfa8`, mantenimiento posterior a la Fase 9,
  ...
  **(1) Funcionalidades implementadas:** dos formas más en «Configuración» → «Cuentas para recibir pagos»: **Bre-B**
  («Tu llave», titular y nombre para reconocerla) y **Otros** («Número o identificador», titular y nombre para
  reconocerla), en un campo de **texto** y nunca en el de teléfono. Se crean, editan, listan, ordenan, archivan,
  vuelven a usar y salen al final del mensaje de los recordatorios sin la etiqueta privada. La llave se guarda **tal
  cual**, sin «@» añadido ni exigido, y solo pierde los espacios exteriores; 1–100 caracteres, una línea, nada
  invisible (BR-M10). Duplicados por el identificador **entero y con mayúsculas**; las formas de siempre, por dígitos,
  como antes (BR-M08). La vista previa del recordatorio ya parte las palabras largas (I-146).
  **(2) Pruebas ejecutadas:** línea base `test:db` 1.373 + 1 y `verify` 1.497; final `verify` **exit 0, 1.522/1.522**,
  `test:db` **1.402 + 1** en 57 archivos (+29), E2E **775/779** con las **8 nuevas en verde**: fallaron dos de I-090,
  una de I-106 y una de «Premios ganados» (I-148); relanzados, **92/93**, y la que falló entonces se reproduce en
  `735eb67`, sin D-209. Además, tres mutaciones detectadas, el ensayo sobre datos existentes **12/12**, el escenario B
  idéntico y `verify-remote` contra la base local **46/46**. Seis errores propios, corregidos, y el fallo de `tsc`
  por el archivo de tipos que deja `next dev` (I-149), registrados en `TEST_RESULTS`.
  **(3) Migraciones:** **74** en local —`0073`: los valores `breb` y `other` del enumerado, solos; `0074`: la columna
  `identifier`, el CHECK de forma con una rama por forma, el CHECK del identificador, el índice de duplicados con una
  rama por forma, las dos funciones de la regla y las dos RPC con `p_identifier`, con su autocomprobación de
  privilegios— y **72 en producción**, sin cambios.
  **(4) Variables de entorno:** ninguna nueva.
  **(5) Problemas que permanecen:** **I-147** (dos altas simultáneas de un vendedor eligen la misma posición; impacto
  bajo, anterior a D-209); **I-148** (dos pruebas de «Premios ganados» que fallan a veces, también sin D-209; impacto:
  una E2E completa puede salir en rojo por ellas); **I-149** (`verify` puede fallar en `tsc` por el archivo de tipos
  que deja `next dev`; impacto: una pasada en rojo que no es del código); `verify:remote` contra producción tendrá
  **dos comprobaciones en rojo a propósito** hasta promover `0073`/`0074`; siguen los de antes —I-140, I-133, I-087,
  I-136, I-142, I-144, la auditoría de I-132 e I-137, I-075, I-090 e I-106—.
  **(6) Qué revisar antes de continuar:** `HANDOFF` §1.a; **D-209**; BR-M10; el orden de promoción, **sin ejecutar**,
  en `DEPLOYMENT` §2.2; y que la base va **antes** que el código.

  Antes, ese mismo día — **Historial de premios ganados EN PRODUCCIÓN (puertas 1 y 2 hechas; la 3 desplegada y
  detenida por el CI, I-140)** (D-208, sin migración nueva; con autorización expresa del dueño, adelantada por él a
  ejecución inmediata).
  Los seis puntos de `CLAUDE.md` §34.3:
  **(1) Funcionalidades implementadas:** las mismas de las Etapas 1 a 3, ahora **en producción**: «Premios ganados» en
  los dos portales, las líneas de las fichas y la cobertura. Las seis migraciones `0067`–`0072` se aplicaron el
  2026-09-18 de 23:57:52 a 23:58:31 UTC; los dos premios que confirmó el dueño —Bogotá 2862 y Cundinamarca 4820,
  «Premio diario», $500.000 cada uno— se reconocieron el 2026-09-19 a las 00:01 UTC por el cargador; y
  `318357ce0139e93ded27cf23cb8416b75e7f8bcc` se desplegó a las 00:04:55 UTC (`dpl_Fn6UBZjA6vTPbjViHDaV6GGWuemE`).
  **(2) Pruebas ejecutadas:** en producción, siete comparaciones por fila con la «Opción A», todas CONTINUAR con
  salida 0; tres sondas; el respaldo validado restaurándolo en local; `verify:remote` **44/44** después de las
  puertas 1 y 3; la comprobación en vivo en verde (30/30 rutas, 7/7 cabeceras, 0 secretos, la versión de `318357c`
  servida); ningún error de ejecución. **El CI de `318357c` terminó en rojo**: 1 prueba de 1.374, la de **I-140**, por
  el orden de las suites en el runner; la verificación del código, en verde. Detalle en `TEST_RESULTS`.
  **(3) Migraciones:** **72** en local y **72 en producción**: los dos entornos, a la par.
  **(4) Variables de entorno:** ninguna nueva.
  **(5) Problemas que permanecen:** **I-140** pone en rojo el CI de `main` —defecto de la prueba, no del producto—;
  la puerta 3 **no se dio por cerrada** y **no se revirtió**. Siguen I-133 (10/08–24/08), I-087, I-136, I-142,
  I-144, la auditoría general de I-132 e I-137, e I-075 e I-090 en la E2E.
  **(6) Qué revisar antes de continuar:** `HANDOFF` §1.a; `RUNBOOK` §9 —aviso de cabecera y estado de las puertas—;
  `DEPLOYMENT` §3.2.l; I-140. **Siguiente:** la decisión del dueño sobre I-140 y los recorridos con sesión de
  `RUNBOOK` §9.9.

  Antes, ese mismo día — **Historial de premios ganados, ETAPA 4: la preparación de la promoción**
  ...
  **(6) Qué revisar antes de continuar:** D-208, **sección «Etapa 4»**; `RUNBOOK` **§9**, entero; `DEPLOYMENT`
  ...
  producción**).
  Los seis puntos de `CLAUDE.md` §34.3:
  **(1) Funcionalidades implementadas:** ninguna nueva: **correcciones mínimas de siete hallazgos demostrados**
  y el **procedimiento de promoción** (`RUNBOOK` §9, preparado y **no ejecutado**). (A) El aviso de cobertura
  **desmentía un premio conservado** cuyo resultado entró en conflicto: ahora dice «con el resultado sin
  confirmar o por verificar» y «Puede que esos sorteos tengan premios que no aparecen aquí», con el alcance de
  la organización ante cualquier filtro. (B) El personal **no podía elegir** en «Vendedor» a quien vendió y hoy
  es Administrador: `0070` —`admin_prize_award_sellers()`— y la opción «Nombre (ya no vende)», sin enlace a
  ficha y sin ampliar el portal del vendedor. (C) La E2E del aviso calculaba su expectativa con la misma
  función que lo produce: ahora cuenta con su propia consulta y escribe el texto a mano. (D, E) **H5-04** y
  **H8-08** observan el bloqueo con `pg_blocking_pids` en vez de deducirlo del tiempo. (F) **I-141**: `anon`
  obtenía el inicio operativo por PostgREST pese a su `REVOKE`, porque la función inmutable se plegaba en un
  plan reutilizado: `0071` la hace `stable security definer`. (G) **I-143**: ensayando en local con el
  privilegio por defecto de producción, `current_seller_org_ids()` —que crea la `0068`— nacía ejecutable por
  `service_role`: `0072` repite su permiso y comprueba la matriz de las 15 funciones al aplicarse.
  **(2) Pruebas ejecutadas:** línea base `verify` exit 0 (1.418) y `test:db` **1.328/1.331** —los 3 fallos,
  Mailpit sin puerto publicado por el entorno; con un proxy local temporal, **22/22**—. Cada hallazgo se **demostró
  antes** de corregirse —A, B, F y G con una prueba o un ensayo que fallaba; C, D y E leyendo la prueba y
  midiendo su tiempo— (el detalle, en `TEST_RESULTS`). El ensayo del escenario B con `0067`–`0072`, idéntico
  al normal, y `verify-remote` contra esa base local, **44/44**. Auditado **sin defecto**: la matriz de acceso por
  PostgREST de nueve personas (**H13**), lo ajeno igual a lo inexistente, los parámetros manipulados y el RSC
  sin un dato de cliente, las correcciones de la Etapa 1, el **volumen** —3.000 boletas, 328 premios, un
  modelo del motor en TypeScript como expectativa independiente, suite nueva de **8**— y **42** pantallas a seis
  anchos. Rendimiento: medianas de **4 a 14 ms** por lectura; el plan crece lineal; **ningún índice**. Al
  cierre: `verify` **exit 0** con **1.422/1.422** unitarias en 78 archivos (**+4**), `test:db` **1.347 y 1 omitida
  a propósito en 54 archivos (+17)** —H12-03, una prueba mía, falló una vez por elegir su sorteo sin orden y se
  corrigió reforzándola— y la E2E completa **768/771**: los 3 son **I-075** (dos de `back-navigation`, con el
  servidor recién arrancado) e **I-090**, ajenos al historial, y sus archivos solos dan **27/27**; las **23** del
  historial, en verde. Cuatro errores propios y dos tropiezos
  del entorno, corregidos y registrados. `verify:remote` **no se corrió**: el encargo prohíbe leer producción.
  **(3) Migraciones:** **72**. Nuevas, **solo en local**: **`0070`** —`admin_prize_award_sellers()`, quién
  aparece como vendedor en el historial del personal, solo perfil y nombre—, **`0071`** —`prize_award_history_start()`
  pasa a `stable security definer`, mismo valor y privilegios— y **`0072`** —repite el permiso de
  `current_seller_org_ids()` nombrando a `service_role` y comprueba la matriz de las 15; en local no cambia
  ningún privilegio—. `0067`–`0069` **no se tocaron**.
  **(4) Variables de entorno:** ninguna nueva para la aplicación. `PREMIOS_EXPLAIN=<archivo>` es **opcional y
  solo de pruebas**: enciende la medición V5-01.
  **(5) Problemas que permanecen:** **I-141** e **I-143** resueltas en local y, con **I-138** e **I-139**,
  pendientes de promoción; **I-140**, nueva —una prueba de privacidad que solo pasa en la primera pasada; defecto de la
  prueba, anterior al encargo, **no** es una fuga—; **I-142**, nueva —limitación: una coincidencia del sistema de
  siempre que se confirme después no aparece hasta reconocerla; hoy sin casos—; **I-133** —el tramo 09/08–24/08
  sigue **pendiente de información**, sin inventar premios ni leerlo como cero—; **I-136** y la **auditoría
  general de I-137**, fuera del alcance a propósito; **I-087**. `verify:remote` tendrá **tres** comprobaciones en
  rojo a propósito hasta la puerta 1. **Ningún problema de privacidad o integridad del historial queda abierto.**
  **(6) Qué revisar antes de continuar:** D-208, **sección «Etapa 3»**, `RUNBOOK` **§9** —el orden, las cuatro
  puertas y lo que no se hace nunca—, I-140..I-142, BR-J21/BR-J22, `TESTING` §4.12 y el §1.a de `HANDOFF`. **La
  promoción está preparada, no autorizada**: la Etapa 4 empieza por comprobar en solo lectura el estado real
  (§9.1) con la línea base por fila de §9.0 y, antes de la puerta 2, construye y prueba el modo de producción
  del cargador (§9.4). **Nada se escribió en producción.**

  Antes, el 2026-09-17 — **Historial de premios ganados, ETAPA 2: las pantallas** (**D-208**, migración
  **`0069`**, **solo en local**). Los seis puntos de `CLAUDE.md` §34.3:
  **(1) Funcionalidades implementadas:** `/seller/prizes` —los premios de los clientes del vendedor, con
  rifas cerradas y clientes archivados, filtros de rifa, fechas del sorteo y cliente en la URL, los cuatro
  indicadores de la base y tabla desde `xl` o tarjetas por debajo—; el resumen de una línea y «Ver
  premios» en la ficha del cliente; `/owner/prizes` —toda la organización con el vendedor, también
  desactivado, y **sin un solo dato de cliente**—; y los cuatro indicadores con «Ver sus premios» en la
  ficha del vendedor. «Premios ganados» entra en el menú de los dos portales sin tocar la barra inferior.
  Cada pantalla distingue error, sin premios, información pendiente y valor pendiente. La **`0069`**
  corrige la cobertura: ya no cuenta sorteos cancelados o suspendidos ni los de rifas en borrador o
  anuladas (I-138). Además, H10-01 ya no espera 800 ms fijos y `HANDOFF` deja de autorizar editar la
  `0068`.
  **(2) Pruebas ejecutadas:** línea base `test:db` **1.329/1.329** y `verify` exit 0 (1.381). El defecto
  de la cobertura **falló con la `0068`** (esperado 8, recibido 12) antes de corregirlo. Al cierre:
  ...
  **(3) Migraciones:** **69**. La nueva es **`0069`**: redefine el cuerpo de `prize_award_coverage()` con
  ...
  tocaron**.
  **(4) Variables de entorno:** ninguna nueva.
  **(5) Problemas que permanecen:** **I-138** e **I-139**, nuevas y resueltas en local, pendientes de
  promoción y despliegue; **I-133** —el tramo 09/08–24/08 sigue pendiente de información del dueño, y la
  pantalla lo dice—; **I-136** y la **auditoría general de I-137**, fuera del alcance a propósito; **I-087**.
  `verify:remote` tendrá **dos** comprobaciones en rojo a propósito hasta promover `0067`–`0069`.
  **(6) Qué revisar antes de continuar:** D-208, **sección «Etapa 2»**, BR-J22, `ARCHITECTURE` §8.28,
  `UX_COPY_GUIDELINES` (Anexos A y B), `TESTING` §4.12 y el §1.a de `HANDOFF`. **La Etapa 3 —auditoría—
  necesita autorización propia**, y promover y desplegar son puertas aparte que **nadie ha pedido**. **Nada
  se escribió en producción.**

  Antes, ese mismo día — **Historial de premios ganados, ETAPA 1 CORREGIDA** (**D-208**, migración
  **`0068`**, **solo en local**). Los seis puntos de `CLAUDE.md` §34.3:
  **(1) Funcionalidades implementadas:** la corrección de siete hallazgos de revisión, **todos
  reproducidos antes de tocarlos**. La unicidad de un reconocimiento pasa a ser **parcial** —solo uno
  vigente, así que **anular y volver a registrar es posible** y la historia se conserva—; el informe del
  cargador corresponde a lo **almacenado** y una petición incompatible **se rechaza nombrando las dos
  cifras**; se rechazan los **duplicados dentro de una petición** y dos ejecuciones a la vez se
  **serializan**; la vista previa anticipa modo, límites, ambigüedad por título y el suelo del historial;
  las dos lecturas del vendedor y **las dos políticas** exigen **rol de vendedor activo**
  (`current_seller_org_ids()`); `numbers_changed` compara contra el campo **fotografiado**; el origen
  **declarado** ya no presenta las condiciones de hoy como históricas; y **la carrera de I-134 queda
  cerrada** con un disparador diferido sobre `lottery_ticket_matches`, **sin tocar el motor**. Añade el
  contrato de lectura de la Etapa 2 (**BR-J23**), el **suelo** del historial en la base
  (`prize_award_history_start()`) y la **cobertura pendiente** (`prize_award_coverage()`).
  **(2) Pruebas ejecutadas:** `test:db` **1.329/1.329** en 53 archivos —la suite del historial pasa de
  35 a **59**—, `verify` **exit 0** con **1.381/1.381** unitarias, y `verify:remote` **41 verde + 1
  rojo a propósito**. Cada hallazgo tiene su evidencia de antes y después en `TEST_RESULTS`, y su
  reproducción determinista —la carrera, con dos conexiones y un orden fijo—. **Dos errores de método
  propios** quedaron registrados: medir RLS con `postgres` no mide nada (`BYPASSRLS`), y una
  serialización que solo retrasa no es una garantía.
  **(3) Migraciones:** **68**. La nueva es **`0068`** —índice parcial, `current_seller_org_ids()`, dos
  políticas acotadas, la defensa de la fotografía, el suelo y la cobertura, y las seis funciones del
  historial rehechas—, **aplicada solo en local**. La `0067` **no se reescribió**.
  **(4) Variables de entorno:** ninguna nueva.
  **(5) Problemas que permanecen:** **I-133 acotada** —el tramo 09/08–24/08 sigue **pendiente de
  información**, y reconocerlo necesitará una **ampliación autorizada aparte**, porque el cargador exige
  una fotografía existente y ahí no hay resultados—; **I-136** precisada; **I-137 nueva** y **anterior**
  a este encargo, con su alcance medido; **I-134 y I-135 resueltas en local** y pendientes de promoción.
  Sigue **I-087**.
  **(6) Qué revisar antes de continuar:** D-208 **entera, empezando por la corrección del final**,
  `BR-J17`..`BR-J23`, `BR-I16`, `TESTING` §4.12 y el §1.a de `HANDOFF`. **La Etapa 2 —las pantallas—
  necesita autorización propia**, y promover `0067` y `0068` es una puerta aparte que **nadie ha
  pedido**. **Nada se escribió en producción.**

  Antes, ese mismo día — **Historial de premios ganados, ETAPA 1 de 4: base, lecturas y los dos
  premios reconocidos** (**D-208** cerrada y aprobada, migración **`0067`**, **solo en local**). Los seis
  puntos de `CLAUDE.md` §34.3:
  **(1) Funcionalidades implementadas:** `declared_prize_awards` —el premio que reconoce el negocio sobre
  una coincidencia que el motor no puede premiar— con sus tres defensas y su inmutabilidad;
  `prize_award_rows` como **única definición** de qué es un premio ganado, que compone los enlaces del
  motor con los reconocimientos sin duplicar y sin que las alternativas multipliquen importes; las
  **cuatro lecturas** (`seller_prize_awards`, `seller_prize_award_totals`, `admin_prize_awards`,
  `admin_prize_award_totals`) con los **cuatro indicadores** calculados en PostgreSQL sobre todo el
  filtro; el cargador `record_declared_prize_awards` —vista previa por omisión, entera o nada,
  idempotente, solo service role— con su script `scripts/record-prize-awards.ts`, que **se niega sin
  `--local`**; y el cerrojo de **BR-I16**: los números de una boleta con coincidencias no cambian por
  ninguna vía. Reglas **BR-J17..BR-J22** y **BR-I16**. **Sin pantallas**: la Etapa 2.
  **(2) Pruebas ejecutadas:** `test:db` **1.305/1.305** en 53 archivos (**+35**), `verify` **exit 0** con
  **1.381/1.381** unitarias (**+6**), y `verify:remote` **41 en verde y 1 en rojo a propósito** —las 10
  funciones de `0067` «no existen» en el proyecto real porque no está promovida—. El cargador se ensayó
  **de punta a punta en local** con un fixture que reproduce producción: vista previa → aplicar
  (`reconocido`, **$1.000.000**) → repetir (`ya estaba`, sin duplicar). **Seis errores propios**
  encontrados y corregidos por el camino, incluidos `position` como palabra reservada y un informe que
  decía «ya estaba» en la primera aplicación; detalle en `TEST_RESULTS`.
  **(3) Migraciones:** **67**. La nueva es **`0067`** —tabla, tres defensas, inmutabilidad, RLS, seis
  funciones, dos disparadores sobre `tickets` y los privilegios de las 10—, **aplicada solo en local**.
  **(4) Variables de entorno:** ninguna nueva.
  **(5) Problemas que permanecen:** **I-133 acotada** —el tramo 09/08–24/08 sigue **pendiente de
  información**, por respuesta del dueño, y la `0067` falta en producción—; **I-134 e I-135 resueltas en
  local** y pendientes de promoción, con el hueco residual de concurrencia **medido y marcado** en la
  lectura; **I-136 nueva** e informativa: corregir un reconocimiento exige anular y volver a registrar, y
  ninguna de las dos acciones tiene pantalla. Sigue **I-087**: el motor no se ha ejercitado en producción,
  así que el origen `engine` está probado solo con datos locales.
  **(6) Qué revisar antes de continuar:** D-208 **entera, empezando por su corrección final**, las nuevas
  `BR-J17..BR-J22` y `BR-I16`, `TESTING` §4.12 y el §1.a de `HANDOFF`. **La Etapa 2 —las pantallas—
  necesita autorización propia**, y promover la `0067` a producción es una puerta aparte que **nadie ha
  pedido**. **Nada se escribió en producción.**
  Antes, ese mismo día — **Historial de premios ganados, ETAPA 0 de 4: contrato propuesto y
  cobertura medida** (**D-208**, entonces una propuesta sin aprobar; sin migración y sin código).
  Los seis puntos que exige `CLAUDE.md` §34.3:
  **(1) Funcionalidades implementadas: ninguna.** El entregable es el contrato propuesto —pantallas,
  rutas, permisos, contabilización y estrategia histórica— y el diagnóstico de cobertura. No se creó ni
  se modificó nada de `src/`, `supabase/` ni `tests/`, y las `BR-J17`+ que propone **no existen todavía**.
  **(2) Pruebas ejecutadas:** `test:db` **1.270/1.270** en 52 archivos y `verify:remote` **41/41**, las
  dos en verde; **5 sondas de solo lectura** contra el proyecto real, validadas antes en local, donde
  dos fallaron por nombrar columnas inexistentes (`raffle_prize_transitions.created_at`, que es
  `transitioned_at`, y `audit_logs.user_id`, que es `actor_profile_id`) y se corrigieron antes de tocar
  producción. `verify` **exit 0** (unitarias **1.375/1.375** en 75 archivos), corrido para dejar
  constancia de que el entorno está sano; **la E2E no se corrió**: no cambia ni una pantalla. Evidencia
  en `TEST_RESULTS`.
  **(3) Migraciones:** ninguna nueva. Siguen siendo **66**, `0066` la última, iguales en local y en el
  proyecto real. **(4) Variables de entorno:** ninguna nueva; las sondas usan `SUPABASE_DB_URL`, que ya
  existía. **(5) Problemas que permanecen:** **I-133** —el período que pide el encargo **no se puede
  cubrir** con los datos del motor: hoy hay **2** coincidencias y **0** premios registrados, y las dos
  conservan el sistema de siempre—, **I-134** —el personal puede editar los números de una boleta que ya
  salió en un sorteo— e **I-135** —un resultado confirmado puede pasar a `conflict` y un historial que
  filtrara por `confirmed` borraría registros—. Sigue **I-087**: el motor configurable no se ha
  ejercitado en producción. **(6) Qué revisar antes de continuar:** **D-208 completa**, sus seis
  preguntas al dueño (H1 a H6) y el §1.a de `HANDOFF`. **La Etapa 1 está bloqueada** hasta que el dueño
  las responda: H1 y H2 deciden si el período pedido se puede cubrir. **Nada se escribió en producción.**
  Antes, ese mismo día — **Premios configurables, ENTREGA 5 de 5: COMPLETA EN PRODUCCIÓN** (D-204 a
  ...
  **1.141/1.141** y E2E **681/683**, con los 2 fallos conocidos de **I-090**, verdes en aislamiento;
  ...
  recargar; en vivo, el identificador nuevo servido, 23/23 rutas y 7/7 cabeceras. Al comprobarlo se abrió **I-115**: un corte de PostgREST cierra la sesión de quien navega y
  ...
  **1.097/1.097** y E2E del catálogo público **58/58**; en vivo, el identificador nuevo servido, 23/23 rutas —el catálogo inexistente en 404— y 7/7 cabeceras.
  ...
  **Ninguna migración, consulta financiera, política RLS ni regla de negocio cambia**, y «Estado de
  ...
  diálogo dentro**. **La operación que el diálogo celebraba era la que lo destruía.** Ahora lo monta
  `ClientCreatedProvider` en el **layout del portal del vendedor**, que ninguna operación apaga, y las
  ...
  **APLICADA al proyecto real y DESPLEGADA** el mismo día.
  ...
  la otra buscaba una clase que la Wave 2 había renombrado, de modo que agotaba su tiempo **sin llegar
  a comprobar nada**. **No se repone ningún `hover` y no se crea ningún token:** pulsar una opción ya
  elegida **no hace nada**, así que pintarla al pasar por encima anunciaría una acción inexistente y
  devolvería el riesgo de I-033. La regla —«lo elegido manda sobre el cursor», que ya seguían las tres
  listas con elección— queda escrita en `OptionList`, junto con la condición que la haría cambiar.
  **El comportamiento del producto no cambia.** **Sin desplegar.**
  Antes, el mismo día: **dos arreglos sobre «Estado de cobro»** (D-173). **(1)** La insignia
  decía «21 boletas en total» junto a «10 disponibles · 6 vendidas» y dejaba cinco sin explicar:
  `tickets_total` es `count(*)` y cuenta además borradores, pendientes de aprobación y anuladas. Ahora
  dice **«16 boletas activas»**, exactamente lo que suman las dos cifras de al lado; el recuento
  completo conserva su sitio en «Mis boletas» con su propio nombre, **«Registradas»**. **(2)** El
  reparto de «Falta cobrar» **acotaba** las cifras para que la suma cuadrara; ahora es una
  **derivación exacta** que garantizan `payment_status` —columna generada— y los CHECK de `tickets`, y
  cuando los datos no la sostienen **la pantalla no escribe la ecuación ni ninguna cifra por estado**:
  se queda con el total autoritativo. **Ninguna migración, política, consulta ni regla financiera
  cambia.** La suite E2E completa deja **cuatro fallos preexistentes y clasificados**: dos de **I-090**
  —acumulación, 39/39 en aislamiento— y dos de **I-101**, nuevo y ajeno: la activación de marca del
  sistema de diseño le quitó el `hover` a una opción ya elegida y no lo repuso. **Sin desplegar.**
  Antes, el mismo día: **`I-099` resuelto en la raíz** (D-172), y su alcance era mucho mayor
  de lo que se escribió al abrirlo. `cn` decidía qué era un `text-*` mirando la escala `text` del tema
  de Tailwind, y los 14 roles tipográficos de este proyecto son extensiones de tema: no los conocía,
  los daba por colores y **descartaba uno de los dos en silencio**. En producción, hoy, eso significa
  que **los mensajes de error de formulario no salen en rojo**, los **botones miden 16 px en vez de
  14**, las **insignias 14 en vez de 12**, el texto secundario de siete componentes **no está
  atenuado** y **`/seller/team` se desborda 14 px a 320 px**. Se arregla con una línea de
  configuración de `tailwind-merge` en `src/lib/utils.ts` —que describe lo que los roles ya son, sin
  inventar ninguna categoría— y el rodeo con `clsx` que había quedado en el panel se retira: vuelve a
  haber **un solo** ayudante de composición de clases. **Ningún token, nombre de rol, valor del tema
  ni API de componente cambia; ninguna migración, política, consulta ni regla financiera tampoco.**
  Verificado midiendo la pantalla antes y después con `getComputedStyle`, y el desbordamiento con una
  A/B sobre la misma base sembrada. Queda además una **regla de trabajo** en `HANDOFF` §1.b: la
  infraestructura de interfaz compartida se verifica con `verify` **más** las pruebas E2E que le
  correspondan, porque un defecto de clases compuestas no se ve con `grep` ni con pruebas unitarias.
  **Sin desplegar.**
  Antes, el mismo día: **el panel del vendedor dice el dinero en una sola sección**
  (D-171). «Resumen financiero» y «Cobranza» eran dos tarjetas contando lo mismo de dos formas, y
  una de sus cifras engañaba: «Abonadas $15.640.000» se lee como dinero abonado y era el **valor de
  venta** de esas boletas. Ahora hay **«Estado de cobro»**, a lo ancho del panel: el inventario en
  el encabezado, cuatro cifras de dinero —**Total vendido**, **Ya cobraste**, **Falta cobrar** y
  **Avance del cobro**, con su barra— y, debajo, «Boletas vendidas según su pago» repartido en dos
  grupos: **Falta cobrar**, con «Sin pagos» y «Con abonos» —esta con sus dos cifras de verdad,
  «Todavía deben» y «Ya abonaron»— y la igualdad `$44.760.000 + $8.700.000 = $53.460.000` **escrita
  a la vista**, y **Pagadas**. Los tres grupos siguen llevando a la lista ya filtrada. **No cambia
  ninguna consulta, migración, política ni regla de negocio, y ninguna cifra cambia de
  significado**: es presentación, y lo único que se añade al modelo es `pendingBy` en
  `collection-breakdown.ts`, un módulo puro con sus pruebas. El indicador **«Recaudado»**, el único
  que depende del selector de fechas, sigue aparte y sin tocar. Verificado midiendo la pantalla a
  **320, 375, 768 y 1360 px** —cero desbordamiento— y con los **34 textos** de la sección por encima
  de AA (mínimo 5,36:1). De paso quedan anotados **I-099** —`cn` descarta en silencio los roles
  tipográficos del sistema de diseño cuando comparten elemento con un rol de color— y **I-100**
  —`DonutChart` se quedó sin consumidores y no se elimina—. **Sin desplegar.**
  Antes: **el paz y salvo de una boleta se registra con un interruptor**
...
| **Pendiente** | Ninguna fase. **Bre-B y «Otros» en las cuentas para recibir pagos** (D-209, `0073` y `0074`): implementado y probado **solo en local**; su promoción —base primero, código después, `DEPLOYMENT` §2.2— **espera autorización**. **Historial de premios ganados**: las **etapas 0 a 3** están entregadas —la 1 corregida, la 2 con sus pantallas y la 3 con la auditoría y el procedimiento de promoción— (D-208 aprobada, migraciones `0067`–`0072` **solo en local**) y la **Etapa 4** —producción, por puertas (`RUNBOOK` §9)— **espera autorización**. Mantenimiento no activo I-030, I-037 e I-046–I-052; prerrequisitos operativos I-021, I-023 e I-024 |
| **Bloqueada** | Ninguna fase. Fuera del plan de fases, el **historial de premios ganados está en producción** (2026-09-19): las puertas 1 y 2 terminaron; la 3 está desplegada y en verde en vivo, pero su **CI en rojo por I-140** la dejó sin cerrar. Corregir esa prueba y desplegarla, o aceptarla, es decisión del dueño (`HANDOFF` §1.a) |
...
como el sistema no admitía el cero, **tuvo que dejarlo en $1**. **Ya en producción** el mismo día:
`0042` aplicada tras respaldo, `ef7bf62` desplegado y el build servido verificado.
...
| Archivo | Qué hace |
|---|---|
| `0042_payment_allocation_zero.sql` | Los `CHECK` de fila de `payments.total_amount` y `payment_allocations.amount` pasan a `>= 0`; dos disparadores `BEFORE INSERT` conservan el `> 0` del alta (BR-F03); `update_payment_allocation` acepta cero y rechaza el negativo con un mensaje propio. **Aplicada al proyecto real el 2026-09-01**, tras el respaldo `Rifas-backups/2026-09-01-pre-0042/`. No escribe ni una fila |
...
1. **`0042` ya está aplicada en los dos entornos** y `ef7bf62` desplegado. Nada pendiente de promover.
2. **El abono de $1 que existe en producción no se tocó** — es exactamente **uno**, comprobado tras
   aplicar. Es un dato real. Con el cambio ya desplegado, el propio vendedor puede dejarlo en $0
   desde la pantalla; hacerlo por migración requiere autorización expresa.
3. **Falta una comprobación humana:** editar un abono a $0 **desde la pantalla en producción**. Vive
   tras el inicio de sesión y un agente no entra con cuenta real (I-066).
4. **BR-F03 ya no es «cero prohibido» a secas.** Distingue alta de corrección. Si tocas la validación
...
## Mantenimiento post-9 — premios configurables, **ENTREGA 5 de 5**: auditoría local y preflight, DETENIDA (D-205, 2026-09-16)

Autorizada expresamente, **la Entrega 5 completa** con dos puertas. **No es una Fase 10** y no lleva
etiqueta `fase-*`. Se hizo la parte local entera y el preflight de **solo lectura** del proyecto real, y
ahí se detuvo: **las dos condiciones de parada del encargo se cumplen** y ninguna se puede resolver sin el
dueño. **La primera puerta no se alcanzó.**

> **PRODUCCIÓN SIN CAMBIOS.** Sigue en la `0057`, sirviendo `c48437a`. Sin respaldo nuevo, sin migraciones,
> sin push, sin despliegue y sin vista previa de la transición. La rifa real sigue en `legacy`.

### 1. Funcionalidades implementadas

| Bloque | Qué hay |
|---|---|
| **I-128, resuelta** | J13 en **2054**, con el calendario de 2026 y sin compartir fechas con otra suite; demostrado con el reloj de la base local simulado antes, durante y después de noviembre y diciembre de 2026 |
| **La puerta de producción del script** | `raffle-prize-transition-guard.ts`: destino explícito y de verdad remoto; para aplicar en producción, la huella de una vista previa anterior —que el script repite y compara—, `--apply` y el identificador de la rifa escrito dos veces. Una respuesta incierta no se da por fallida ni se repite. Sin identificadores de producción en el código |
| **`verify:remote`** | +3 comprobaciones de solo lectura: las cinco tablas de premios, las seis RPC para `authenticated` y no para `anon`, y nada más que `SELECT` para las sesiones. **34/34** contra la base local |
| **El preflight de producción** | Sondas de solo lectura (`build/e5/`, fuera de Git): inventario, la rifa candidata por identificador y organización, sus conteos de control y huellas, la programación, los resultados, las membresías que recibirían el aviso y la réplica de la comprobación de I-127, probada idéntica a la función de la `0063` |

### 2. Pruebas ejecutadas y resultados

| Comando | Resultado |
|---|---|
| `db:reset` · `seed:local` | ✅ `0001`–`0063` |
| J13 con el reloj simulado (8 fechas) | La anterior falla desde el 02/11/2026; la nueva, **6/6** en las ocho. Archivo completo **94/94** |
| `raffle-prize-transition-guard.test.ts` y cuatro mutaciones de la puerta | ✅ **26/26**; cada mutación, detectada por **una** prueba |
| Ensayo local del script —rechazos, vista previa, aplicación y reintento— y de la sonda de verificación posterior | ✅ sin escribir nada en los rechazos; 1 transición, 6 premios, 4 avisos y 1 fila de bitácora al aplicar; nada al repetir; sonda en verde |
| Banco de rendimiento con 5.000 boletas, en una transacción deshecha | Transición 44–50 ms; motor configurable 34–43 ms (102 enlaces, 4 llamadas como máximo por función); heredado 20–73 ms, sin diferencia medible con la defensa nueva |
| `npm run test:db` | ✅ **1.230/1.230** en 50 archivos (81 s) |
| `npm run verify` | ❌ → ✅ `tsc` TS2559 en el script, corregido; lint **0 errores** (2 avisos preexistentes), **1.363/1.363** unitarias en 74 archivos, build |
| `npm run test:e2e` completa | **742/744** en 43,1 min: `back-navigation:25` (**I-075**) y `ventas-por-fecha:163` (**I-090**, «recibido 54»); aislados **9/9** y **18/18** |
| Preflight: `migration list`, `db push --dry-run` y las dos sondas | `0058`–`0063` pendientes y nada más; la rifa termina el **01/11/2026**; **25** sorteos sin resultado confirmado |
| `git diff --check` · SHA-256 de los dos archivos protegidos | ✅ · ✅ iguales al inicio |

**Errores encontrados y corregidos:** TS2559 al pasar `process.env` entero a la puerta; un filtro `-t "J13"`
que arrastraba J2-04 en la simulación; y tres fallos del banco de medición —escapes de `psql` perdidos,
rutas convertidas por Git Bash y `auto_explain` solo para `supabase_admin`—. **Ninguno era del producto.**

### 3. Migraciones que existen

**`0001`–`0063` en local; `0001`–`0057` en el proyecto real**, comprobado con `migration list`. **Esta
entrega no crea ninguna.** `db push --dry-run` lista exactamente `0058`–`0063`, y cada una aparece solo en
el commit que la creó.
...
| Asunto | Impacto |
|---|---|
| **I-129: la rifa real termina el 01/11/2026** | Bloquea la transición: ningún premio cabe en sus fechas. Decisión del dueño |
| **I-127: 25 sorteos jugados sin resultado confirmado**, del 27/07 al 24/08/2026 | Bloquea la transición. Esperar no lo resuelve (horizonte de 10 días). Decisión del dueño |
| **I-130: privilegios de `audit_logs_id_seq` más amplios en el proyecto real** | Info: no alcanzable desde la API. Una migración futura con autorización |
| `0058`–`0063` y el código no están en producción | Lo previsto hasta pasar la primera puerta |
| I-075 e I-090 en la E2E completa | Pruebas, no producto; verdes en aislamiento |

### 6. Qué debe revisar el siguiente agente antes de comenzar

1. **Las decisiones del dueño sobre I-129 e I-127**, antes que nada. Sin ellas no hay nada que aplicar.
2. **Repetir el preflight de solo lectura** (`RUNBOOK` §8.2, la réplica si la `0063` aún no está) justo antes
   de la primera puerta: los conteos y la lista de sorteos cambian con la operación.
3. **Respaldo nuevo** antes de la primera escritura, no el de hoy: no se generó.
4. **La rifa se elige por identificador y organización**: `d64af684-1378-45b9-bb71-2141a58a5013` de
   `ec88961d-7c81-4b27-ae03-d9bccc73eda6`, hoy «SORTEO CAMIONETA KIA 2027».
5. **No se relaja I-127 ni se cambian fechas** sin decisión expresa; una regla nueva para los sorteos
   anteriores a los resultados automáticos sería una migración `0064` con sus pruebas.
6. **El script contra producción** exige `--production`, la huella de una vista previa anterior y
   `--confirm-raffle` (`RUNBOOK` §8.3).
