...
export function reassignBlockedReason(ticket: ReassignEligibility): string | null {
  ...
  if (ticket.hasPaymentHistory) {
    return 'Esta boleta tiene abonos en su historial y ya no puede cambiar de cliente.'
  }
  if (ticket.hasLotteryMatch) {
    return 'Esta boleta ya hace parte de un resultado registrado y no puede cambiar de cliente.'
  }
  return null
}
