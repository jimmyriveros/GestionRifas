...
test.describe('Boletas', () => {
  ...
  test('crea una boleta valida y queda disponible (prueba 6)', async ({ page }) => {
    const numbers = randomTicketNumbers()

    await page.goto('/owner/tickets/new')
    await page.getByLabel('Numero diario').fill(numbers.daily)
    await page.getByLabel('Numero semanal').fill(numbers.weekly)
  ...
  test('el campo no admite mas de 4 digitos (prueba 7, BR-N02)', async ({ page }) => {
    await page.goto('/owner/tickets/new')
    ...
    const weekly = page.getByLabel('Numero semanal')
  ...
  test('rechaza una combinacion ya usada en la rifa (prueba 8, BR-N04)', async ({ page }) => {
    const numbers = randomTicketNumbers()
  ...
  test('rechaza el duplicado tambien cuando es de otro vendedor (prueba 9, BR-N05)', async ({
    page,
  }) => {
    const numbers = randomTicketNumbers()
    ...
    await expect(page.getByText(/Ya existe una boleta con esa combinacion/)).toBeVisible()
  })

  test('la misma combinacion SI se permite en otra rifa (BR-N06)', async ({ page }) => {
    const numbers = randomTicketNumbers()
    await createTicket(refs, {
      dailyNumber: numbers.daily,
      weeklyNumber: numbers.weekly,
  ...
  test('anula una boleta exigiendo motivo (prueba 13, BR-I10)', async ({ page }) => {
    ...
    await page.getByLabel('Motivo (obligatorio)').fill('Numeros mal digitados en la papeleta')
    await page.getByRole('button', { name: 'Anular', exact: true }).click()

    await expectToast(page, /anulada/i)
    await expect(page.getByText('Anulada').first()).toBeVisible()
    await expect(page.getByText('Numeros mal digitados en la papeleta')).toBeVisible()

    // Una boleta anulada ya no ofrece editar ni anular de nuevo.
    await expect(page.getByRole('button', { name: 'Editar numeros' })).toHaveCount(0)
    await expect(page.getByRole('button', { name: 'Anular boleta' })).toHaveCount(0)
  })

  test('edita los numeros de una boleta disponible', async ({ page }) => {
    const numbers = randomTicketNumbers()
    ...
    await page.goto(`/owner/tickets/${ticket.id}`)
    await page.getByRole('button', { name: 'Editar numeros' }).click()
    await page.getByLabel('Numero diario').fill(replacement.daily)
    await page.getByLabel('Numero semanal').fill(replacement.weekly)
  ...
  test('busca por numero exacto sin confundir ceros iniciales (BR-N03)', async ({ page }) => {
    ...
    await page.getByPlaceholder('Codigo interno, numero diario o semanal').fill('0007')
...
test.describe('Proteccion por rol (prueba 14)', () => {
  ...
  test('sin sesion, el portal administrativo redirige al login', async ({ page }) => {
