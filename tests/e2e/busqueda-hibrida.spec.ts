...
test.describe('Listas paginadas: boletas y clientes', () => {
  test('busca sola tras la pausa, sin una petición por tecla', async ({ page }) => {
    ...
    const input = page.getByRole('searchbox', { name: 'Buscar por número de boleta', exact: true })
  ...
  test('limpiar restaura la lista y quita el término de la dirección', async ({ page }) => {
    ...
    await expect(
      page.getByRole('searchbox', { name: 'Buscar por número de boleta', exact: true }),
    ).toHaveValue('')
  ...
  test('cambiar el término vuelve a la primera página', async ({ page }) => {
    ...
    await page
      .getByRole('searchbox', { name: 'Buscar por número de boleta', exact: true })
      .fill('0100')
    await page
      .getByRole('searchbox', { name: 'Buscar por número de boleta', exact: true })
      .press('Enter')
